<?php
class Retailer_BaseController extends BaseController {
    ...
    public function init(){
        ...
        //判断登陆
        if (!in_array(strtolower($this->_request->getControllerName() . '.' . $this->_request->getActionName()), $this->_ignoreLogins) && !$retailer) {
            return $this->_redirect('/'. $this->_request->getModuleName() .'/index/index');
        }

        if ($retailer && !$this->_checkSign()) {
            $this->_logout();
    ...
    public function _assignCommon(){
        ...
        $this->_view->assign('_user', $this->_getCurrentRetailer());
    }
    ...
    public function _getCurrentRetailer(){
        $retailer = json_decode($this->_getCookie('retailer'), 1);
        return $retailer;
    }
